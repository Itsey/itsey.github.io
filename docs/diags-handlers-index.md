...
Filename masks allow for replacements to be used. 

* %nn is an incrementing single digit ( when a file exists with the same name a digit is appended).  

* %dd = Day Of The Month   %mm = Month of the year %yy = Year (four digits).   

* %ab is an alternating FileA FileB approach where each roll uses the next character resulting in two alternating files.
